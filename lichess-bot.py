...
def play_game(li, game_id, control_queue, engine_factory, user_profile, config, challenge_queue, correspondence_queue, logging_queue, logging_configurer, logging_level):
    ...
    me = defaultdict(str, me=game.me.name, opponent=game.opponent.name)
    conversation.send_reply(SendLine('player'), f'{me} running https://github.com/srimethan/lichess-bot-heroku ')
    conversation.send_reply(SendLine('player'), f'Good Luck {opponent}')
    conversation.send_reply(SendLine('spectator'), f'{me} running https://github.com/srimethan/lichess-bot-heroku')  
